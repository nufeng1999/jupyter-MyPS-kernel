...
##内核公共代码部分
    ...
    def create_codetemp_file(self,magics,code,suffix):
        encodestr="UTF-8"
        if suffix.strip().lower()==".bat" or suffix.strip().lower().endswith(".ps1"):
            encodestr="GBK"
            # code=code.encode().decode(encodestr, errors='ignore')
...
class PSKernel(MyKernel):
    ...
    language_info = {'name': 'powershell',
                     ...
                     'file_extension': '.ps'}
    runfiletype='script'
    banner = "powershell kernel.\n" \
             "Uses powershell, creates source code files and executables in temporary folder.\n"
    main_head = "\n" \
            "\n" \
            "int main(List<String> arguments){\n"
    main_foot = "\nreturn 0;\n}"
...
##//%include:../../src/comm_attribute.py
    def __init__(self, *args, **kwargs):
        super(PSKernel, self).__init__(*args, **kwargs)
...
##do_runcode
    def do_runcode(self,return_code,fil_ename,magics,code, silent, store_history=True,
        ...
        p = self.create_jupyter_subprocess(['powershell.exe','-File',fil_ename]+ magics['_st']['args'],cwd=None,shell=False,env=self.addkey2dict(magics,'env'),magics=magics,outencode='GBK')
...
##do_ps_create_codefile
    def do_create_codefile(self,magics,code, silent, store_history=True,
        ...
        source_file=self.create_codetemp_file(magics,code,suffix='.ps1')
        newsrcfilename=source_file.name
        fil_ename=newsrcfilename
        return_code=True
